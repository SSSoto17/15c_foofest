export default function Button({ variant, size, formAction, onClick, isDisabled, children }) {
  const variants = {
  ...
  return (
    <button {...(formAction ? { formAction } : { onClick })} disabled={isDisabled} className={`button ${variants[variant]} ${variants[size]}`}>
      {children}
    </button>
  );
}
